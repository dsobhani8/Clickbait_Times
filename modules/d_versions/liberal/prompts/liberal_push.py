...
# Liberal
class pushLiberal(dspy.Signature):
    """
    --- CONTEXT ---
    ...
    original_title = dspy.InputField()
    original_lead = dspy.InputField()
    push_title = dspy.OutputField(desc="title of the article. Keep it short and do not include anything else than the title. Do not include any prefixes to the title (e.g. 'Title: ').")
    push_body = dspy.OutputField(desc="lead of the article. Keep it concise and do not include anything else than the lead. Do not include anything else than the body. Do not include any prefixes (e.g. 'Body: ' or 'Lead: ').")
     
push_liberal = dspy.Predict(pushLiberal)